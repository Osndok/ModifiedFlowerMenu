...
class MenusPanel extends JPanel implements MouseListener, MouseMotionListener
{
	...
	protected
	MenusPanel(ArrayList<String> itemNames, ArrayList<Integer> itemIDs, String[] groups)
	{
		setLayout(null);
		...
		for (int i = 0; i < noOfItems; i++)
		{
			switch (itemIDs.get(i) % 7)
			{
				case 0:
				case 6:
					menuItems[i] = new Gesture(pigtailCurve, itemNames.get(i), itemIDs.get(i), Location[itemIDs.get(i)],
												  Position[itemIDs.get(i)]);
					break;
				case 1:
				case 5:
					menuItems[i] = new Gesture(cuspedCurve, itemNames.get(i), itemIDs.get(i), Location[itemIDs.get(i)],
												  Position[itemIDs.get(i)]);
					break;
				case 2:
				case 4:
					menuItems[i] = new Gesture(bentCurve, itemNames.get(i), itemIDs.get(i), Location[itemIDs.get(i)],
												  Position[itemIDs.get(i)]);
					break;
				case 3:
					menuItems[i] = new Gesture(straight, itemNames.get(i), itemIDs.get(i), Location[itemIDs.get(i)],
												  Position[itemIDs.get(i)]);
					break;
			}
	...
	void paintComponent(Graphics g)
	{
		...
		if (mode == 1)//Novice
		{
			...
			if (selecteddirection != -1)
			{
				for (int j = 0; j < 7; j++)
				{
					if (idMap[selecteddirection * 7 + j] != -1)
					{
						...
						{
							menuItems[idMap[selecteddirection * 7 + j]].drawGesture(this, g, center, true,
																					   offset[selecteddirection]);
						}
						else
						{
							menuItems[idMap[selecteddirection * 7 + j]].drawGesture(this, g, center, false,
																					   offset[selecteddirection]);
						}
					}
				}
			...
			{
				g.drawLine(inputGesture.get(k).x, inputGesture.get(k).y, inputGesture.get(k + 1).x,
							  inputGesture.get(k + 1).y);
			}
	...
	void mousePressed(MouseEvent e)
	{
		...
		offset[0] = Math.min(Math.min(Math.min(Math.min(Math.min(offset[1], offset[2]), offset[3]), offset[5]),
										 offset[6]), offset[7]);
		offset[4] = offset[0];
		...
		if (nGest.length > 2 && selecteddirection == -1 && selectedgesture == -1)
		{
			...
			distances[2] = DTWDistance(normalize(reSampling(nGest, bentCurve.curvePoints.size()),
													bentCurve.getPointInt(Math.toRadians(45 * selecteddirection),
																			 true)),
										  normalize(bentCurve.getPointInt(Math.toRadians(45 * selecteddirection), true),
													   bentCurve.getPointInt(Math.toRadians(45 * selecteddirection),
																				true)));
			...
			distances[3] = DTWDistance(normalize(reSampling(nGest, straight.curvePoints.size()),
													straight.getPointInt(Math.toRadians(45 * selecteddirection),
																			false)),
										  normalize(straight.getPointInt(Math.toRadians(45 * selecteddirection), false),
													   straight.getPointInt(Math.toRadians(45 * selecteddirection),
																			   false)));
	...
	private
	double LinearDistance(Point[] curveA, Point[] curveB)
	{
		double distance = 0;
	...
	static
	Point[] reverseOrder(Point[] curve)
	{
	...
	private static
	Point[] normalize(Point[] curve, Point[] prototype)
	{
	...
	private static
	Point[] removeConsecDouble(Point[] curve)
	{
	...
	private static
	Point[] reSampling(Point[] curve, int noOfPoints)
	{
		ArrayList<Point> result = new ArrayList<Point>();
		...
		while (curvelength < noOfPoints)
		{
			...
			Point newPoint = new Point((result.get(indLongest).x + result.get(indLongest + 1).x) / 2,
										  (result.get(indLongest).y + result.get(indLongest + 1).y) / 2);
