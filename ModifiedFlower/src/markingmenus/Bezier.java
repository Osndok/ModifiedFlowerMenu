...
class Bezier
{
	...
	public
	Point[] getPointInt(double angle, boolean reflection)
	{
		...
		{
			IntegerPoints[i] = new Point((int) Math.round(curvePoints.get(i).x),
											(int) Math.round(curvePoints.get(i).y));
			if (reflection == true)
			{
	...
	private
	void buildCurve(Point[] controlPoints)
	{
